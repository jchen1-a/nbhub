<?php
// article.php - 100% 完整版 (修复 SELECT id 报错，包含时光机、点赞、评论与暗黑武侠UI)
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['revert_version_id'])) {
        ...
        } elseif ($user_id == $article['user_id']) { 
    ...
    if ($user_id) {
        // 【关键修复】：将 SELECT id 改为 SELECT 1
        $check = $pdo->prepare("SELECT 1 FROM article_likes WHERE article_id = ? AND user_id = ?");
