<?php
// guides.php - 100% 完整版 (极意·墨影流沙 / 电影级暗黑武侠风)
                        ...
                        <?php if(!empty($popular_guides)): ?>
                            <?php $rank = 1; foreach($popular_guides as $guide): ?>
                                ...
                                <a href="article.php?id=<?php echo $guide['id']; ?>" class="wd-boss-card">
                                    ...
                                    <div class="boss-content">
                                        <div class="boss-info">
                                            <div class="boss-tags">
                                                <span class="wd-tag"><?php echo htmlspecialchars(strtoupper($guide['category'] ?? 'GENERAL')); ?></span>
                                                <span class="wd-tag-gold">绝 顶 (MAESTRO)</span>
                                            </div>
                                ...
                                <a href="article.php?id=<?php echo $guide['id']; ?>" class="wd-challenger-card">
                                    <div class="wd-card-hover-line diff-bg-<?php echo $guide['difficulty']; ?>"></div>
                                    <div class="challenger-rank"><?php echo $rank == 2 ? '贰' : '叁'; ?></div>
...
:root {
    --w-bg: #050505;           /* 极夜黑 */
    --w-surface: #0f0f11;      /* 幽冥灰 (卡片底色) */
    --w-text: #e8e6e3;         /* 宣纸白 */
    --w-muted: #6b6b6b;        /* 燃灰 */
    --w-red: #c93838;          /* 丹砂红 */
    --w-gold: #b89f73;         /* 暗金 */
    --w-border: rgba(232, 230, 227, 0.08); /* 极弱的呼吸边框 */
...
/* ==== 深渊背景与印章水印 ==== */
.wd-global-bg { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; z-index: -10; pointer-events: none; background: radial-gradient(circle at 50% 0%, #1a1a1c 0%, var(--w-bg) 70%); }
.wd-ink-stain { position: absolute; top: -10%; right: -5%; width: 50vw; height: 50vw; background: radial-gradient(circle, rgba(201, 56, 56, 0.03) 0%, transparent 60%); filter: blur(60px); }
.wd-watermark { position: absolute; bottom: 5%; right: 5%; font-family: 'Noto Serif SC', serif; font-size: 25vw; font-weight: 900; color: rgba(255,255,255,0.015); line-height: 0.8; user-select: none; }
...
.wd-title { font-size: 5em; letter-spacing: 15px; margin-bottom: 10px; line-height: 1.1; text-shadow: 0 10px 30px rgba(0,0,0,0.5);}
...
.wd-search-line input::placeholder { color: #555; }
...
.wd-btn-primary { background: var(--w-red); color: #fff; border: 1px solid var(--w-red); box-shadow: 0 4px 20px rgba(201, 56, 56, 0.2); }
...
.wd-btn-outline:hover { border-color: var(--w-text); background: rgba(255,255,255,0.05); }
...
.wd-card, .wd-boss-card, .wd-challenger-card, .wd-list-item {
    ...
    transition: all 0.5s var(--w-ease); display: flex; border: 1px solid transparent; border-radius: 4px;
}
.wd-card:hover, .wd-boss-card:hover, .wd-challenger-card:hover {
    background: #141416; border-color: var(--w-border); transform: translateY(-5px);
    box-shadow: 0 15px 40px rgba(0,0,0,0.5);
}
.wd-card *, .wd-boss-card *, .wd-challenger-card *, .wd-list-item * { position: relative; z-index: 2; transition: color 0.3s; } 

/* 悬停时如剑光划过的锐利细线 */
.wd-card-hover-line { position: absolute; top: 0; left: 0; width: 0; height: 1px; transition: width 0.6s var(--w-ease); z-index: 3; }
...
.wd-tag-gold { font-family: 'Noto Serif SC', serif; font-size: 0.8em; font-weight: 700; color: var(--w-bg); background: var(--w-gold); padding: 3px 10px; border-radius: 2px; letter-spacing: 1px;}
...
.wd-boss-bg-glow { position: absolute; top: 0; right: 0; width: 50%; height: 100%; background: radial-gradient(circle at 80% 50%, rgba(201, 56, 56, 0.05) 0%, transparent 70%); z-index: 0; transition: 0.5s;}
.wd-boss-card:hover .wd-boss-bg-glow { background: radial-gradient(circle at 80% 50%, rgba(201, 56, 56, 0.1) 0%, transparent 70%); }
.wd-rank-seal { position: absolute; right: 40px; top: 40px; font-family: 'Noto Serif SC', serif; font-size: 8em; font-weight: 900; color: rgba(255,255,255,0.02); line-height: 1; pointer-events: none; z-index: 1; transition: 0.5s;}
.wd-boss-card:hover .wd-rank-seal { color: rgba(201,56,56,0.05); transform: scale(1.05); }
...
.wd-boss-card:hover .boss-title { color: var(--w-gold) !important; }
.boss-meta { display: flex; gap: 30px; color: var(--w-muted); font-size: 0.9em; font-weight: 300; letter-spacing: 1px; }
.boss-icon-wrap { font-size: 6em; color: var(--w-border); transition: 0.5s; }
.wd-boss-card:hover .boss-icon-wrap { color: var(--w-red); transform: rotate(-5deg) scale(1.1); filter: drop-shadow(0 0 20px rgba(201,56,56,0.3));}

/* --- 榜眼/探花：残影卡片 --- */
.wd-challenger-card { padding: 40px; flex-direction: column; justify-content: flex-end; }
.challenger-rank { position: absolute; top: 20px; right: 30px; font-family: 'Noto Serif SC', serif; font-size: 4em; font-weight: 900; color: rgba(255,255,255,0.03); line-height: 1; transition: 0.4s;}
.wd-challenger-card:hover .challenger-rank { color: rgba(255,255,255,0.08); }
.challenger-content { flex: 1; margin-bottom: 30px;}
.challenger-title { font-size: 1.6em; line-height: 1.4; letter-spacing: 1px; }
.wd-challenger-card:hover .challenger-title { color: var(--w-text) !important; }
...
.wd-card:hover .wd-card-title { color: var(--w-text) !important; }
...
.wd-list-item { align-items: center; padding: 25px 40px; background: transparent; border-bottom: 1px solid var(--w-border); border-radius: 0; }
.wd-list-hover-bg { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: var(--w-surface); transform: scaleY(0); transform-origin: center; transition: 0.4s var(--w-ease); z-index: 0;}
...
.list-title { font-size: 1.3em; letter-spacing: 1px; transition: 0.3s; color: rgba(232, 230, 227, 0.8) !important;}
.wd-list-item:hover .list-title { color: var(--w-text) !important; }
.list-meta { text-align: right; display: flex; flex-direction: column; gap: 6px; z-index: 2;}
.list-author { font-size: 0.9em; font-weight: 300; color: var(--w-text); letter-spacing: 1px;}
...
.wd-error { background: rgba(201, 56, 56, 0.1); border: 1px solid var(--w-red); color: var(--w-red); padding: 15px 20px; margin-bottom: 40px; border-radius: 2px; font-weight: 300; letter-spacing: 1px;}
